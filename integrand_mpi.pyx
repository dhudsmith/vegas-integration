from libc.math cimport sqrt, sin, cos, log
...
cdef double cintegrand(x):
    ...
    cdef double k312    = _sqrt3 * K * cos(alpha)
    cdef double k312p   = _sqrt3 * Kp * cos(alphap)
    cdef double k12     = 2 * K * sin(alpha)
    cdef double k12p    = 2 * Kp * sin(alphap)
